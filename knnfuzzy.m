%%%[UPDATED CODE] - MIL-FCKNN - 23-AUG-2016 [MODIFICATION DONE INSIDE HAUSDROFF DISTANCE] 

%"MIL Fuzzy Citation Knn Algorithm"(Dip Ghosh Sir).... 
% Here we are generating a membership matrix along with
% corresponding classified-labels and original class. 
% Note-here the output is the bag level accuracy.
% NOTE-Functions included here are: reference, citer_label, habsf(Hausdorff Distance).
function [test_bl, test_il, test_bp, test_ip] = knnfuzzy(para, train_bags, test_bags)
global preprocess;
% %global mv;
% % global ctval;
% % global refval;
...
di_mat_tr=0;
    for l=1:ntrainbags
        ...
        di_mat_tr(j,l)=di_mat_tr(l,j);%% please note that here the 
                                        %%hausdroff distance code was modified to
                                        %to counter duplicate instances in
                                        %the bags.
        end
        di_mat_tr(l,l)=999999999;
    end
    u_tr=[];
% for label=1:2         %%correction done,the this line removed..
    for i=1:ntrainbags
        for j=1:ntrainbags
...
%================================================================================================= 
%%From the previous version ...... used for multi-instance multi-label Fuzzy Citation Knn 
...
%         else      %%correction for this line removed..
            u_tr(i,1)=0.49*(size(find(testb_label_sample_tr==(train_bags(i).label)),2)/(ref+citer));
%         end       %%correction for this line removed..
        else
            u_tr(i,2)=0;    %%After correction for this line added..
            u_tr(i,1)=1;    %%After correction for this line added..
        end
    end
% end               %%after correction for this line removed..
% ------------------------------------------------------------------------------------
...
for i=1:ntestbags
   ...
   %Evaluation of test bags
         ...
         for label=1:2
         for u=1:ref  %for references
             ur(u)=(1/di_mat(ntrainbagsnew,y(u)))^(2/(m-1));
             um(u)=u_tr(y(u),label); %this is the place where membership matrix of train bag is used.
         end
         
         un=u+1;cu=1;
         
         for ux=un:(ref+(cm-1)) %citers
