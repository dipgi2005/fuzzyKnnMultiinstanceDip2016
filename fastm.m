...
dist3 = sqrt(bsxfun(@plus,dot(a,a,1)',dot(b,b,1))-2*a'*b); 

elapsed = toc();
fprintf('another matrix took %.3f seconds.\n', elapsed);
...
fprintf('Difference in between first and second result: %f\n', sum(abs(dists1(:) - dist3(:))));
if (exist('OCTAVE_VERSION')) fflush(stdout); end